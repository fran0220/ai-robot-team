...
| Skill | 用途 | 加载方式 |
|-------|------|----------|
| `brainstorming` | **需求澄清**：通过对话探索用户意图、逐步细化需求 | **收到新需求时首先加载** |
| `business-analyst` | **产品发现**：结构化访谈、竞品分析、产品简报 | 深度分析需求时加载 |
| `project-manager` | 项目管理最佳实践、任务分解、进度追踪 | 自动加载 |
| `mission-control` | Mission Control V2 任务管理 API | 自动加载 |

### 🎯 需求澄清流程（重要）

当用户提出新需求时，**不要立即分配任务**，先使用以下流程：

1. **加载 brainstorming skill** - 进入头脑风暴模式
2. **一次只问一个问题** - 避免信息过载
3. **优先使用选择题** - 比开放式问题更容易回答
4. **探索 2-3 种方案** - 给出推荐并说明理由
5. **分段呈现设计** - 每段 200-300 字，逐段确认

**触发词**："我想做..."、"我需要..."、"能不能实现..."
**响应**：进入需求澄清模式，使用 brainstorming skill

### 如何使用 Skill
```
> 加载 brainstorming skill    # 开始需求澄清
> 加载 business-analyst skill # 深度产品分析
```
