...
export async function POST(
  ...
  try {
    // Get task
    const task = await queryOne<TaskBasic>('SELECT * FROM tasks WHERE id = $1', [taskId]);
    if (!task) {
      return NextResponse.json({ error: 'Task not found' }, { status: 404 });
    }

    // Check if already locked
    const existingSpec = await queryOne<PlanningSpec>(
      'SELECT * FROM planning_specs WHERE task_id = $1',
      [taskId]
    );
    ...
    const questions = await queryAll<PlanningQuestion>(
      'SELECT * FROM planning_questions WHERE task_id = $1 ORDER BY sort_order',
      [taskId]
    );
    ...
    await run(`
      UPDATE tasks 
      SET description = $1, status = 'inbox', updated_at = NOW()
      WHERE id = $2
    `, [specMarkdown, taskId]);
    ...
    await run(`
      ...
      VALUES ($1, $2, 'status_changed', 'Planning complete - spec locked and moved to inbox')
    `, [activityId, taskId]);

    // Get the created spec
    const spec = await queryOne<PlanningSpec>(
      'SELECT * FROM planning_specs WHERE id = $1',
      [specId]
    );
