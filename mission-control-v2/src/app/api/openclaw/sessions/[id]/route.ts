...
import type { OpenClawSession, Agent } from '@/lib/types';
...
export async function PATCH(request: Request, { params }: RouteParams) {
  try {
    ...
    const session = await queryOne<OpenClawSession & { agent_id?: string; task_id?: string }>(
    ...
    const updatedSession = await queryOne<OpenClawSession>(
    ...
    if (status === 'completed') {
      if (session.agent_id) {
        await run('UPDATE agents SET status = $1 WHERE id = $2', ['idle', session.agent_id]);
      }
      if (session.task_id) {
        broadcast({
...
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    let session = await queryOne<OpenClawSession & { agent_id?: string; task_id?: string }>(
      'SELECT * FROM openclaw_sessions WHERE openclaw_session_id = $1',
      [id]
    );

    if (!session) {
      session = await queryOne<OpenClawSession & { agent_id?: string; task_id?: string }>(
        'SELECT * FROM openclaw_sessions WHERE id = $1',
        [id]
      );
    }
    ...
    if (agentId) {
      const agent = await queryOne<Agent>('SELECT * FROM agents WHERE id = $1', [agentId]);
      if (agent && agent.role === 'Sub-Agent') {
        await run('DELETE FROM agents WHERE id = $1', [agentId]);
      } else if (agent) {
        // Update non-subagent back to idle
        await run('UPDATE agents SET status = $1 WHERE id = $2', ['idle', agentId]);
      }
    }
