...
import type { Task, Agent, OpenClawSession, Event } from '@/lib/types';
  ...
  try {
    ...
    if (body.session_id && body.message) {
      ...
      const session = await queryOne<OpenClawSession>(
        'SELECT * FROM openclaw_sessions WHERE openclaw_session_id = $1 AND status = $2',
...
export async function GET() {
  try {
    const recentCompletions = await queryAll<Event & { agent_name?: string; task_title?: string }>(
