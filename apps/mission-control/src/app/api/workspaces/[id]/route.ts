...
export async function GET(
  ...
  try {
    // Try to find by slug first, then by ID (if valid UUID)
    let workspace = await queryOne<Workspace>(
      'SELECT * FROM workspaces WHERE slug = $1',
      [id]
    );
    
    // If not found by slug, try by UUID (only if it looks like a UUID)
    if (!workspace && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)) {
