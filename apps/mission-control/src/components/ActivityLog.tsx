...
export function ActivityLog({ taskId }: ActivityLogProps) {
  ...
  const formatTimestamp = (timestamp: string) => {
    ...
    if (diff < 3600000) {
      ...
      return mins > 1 ? t('activity.minutesAgo', { count: mins }) : t('activity.minuteAgo', { count: mins });
    }
    
    // Less than 24 hours
    if (diff < 86400000) {
      const hours = Math.floor(diff / 3600000);
      return hours > 1 ? t('activity.hoursAgo', { count: hours }) : t('activity.hourAgo', { count: hours });
    }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-mc-text-secondary">{t('activity.loadingActivities')}</div>
      </div>
    );
  }
